...
class EndOrdersTeam: UIViewController {
...
//        }
    ...
    func filterOrders(dateFrom : String){
        ad.isLoading()
        APIClient.filterOrders(date_from: dateFrom, completionHandler: { (state, data) in
            ad.killLoading()
            print(state)
            guard state else {
                   self.showDAlert(title: "Error".localized, subTitle: "tryAgain".localized , type: .error,buttonTitle: "tryAgain".localized, completionHandler: nil)
...
extension EndOrdersTeam : UITableViewDelegate , UITableViewDataSource {
...
extension EndOrdersTeam : UIPickerViewDelegate, UIPickerViewDataSource {
